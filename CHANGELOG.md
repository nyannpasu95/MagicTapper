...
- **灵敏度设置窗口（Sensitivity Settings…）**：可在菜单栏打开设置窗口，
  实时调节轻点识别阈值（最大点击时长、光标移动容差、右键按住时长、
  双击窗口、左右分界、表面移动阈值），修改即时生效并持久化；
  此前这些参数只能手动编辑 UserDefaults，README 中的旧说明已失效
- **设备事件驱动检测**：新增 `MultitouchDeviceMonitor`，通过 IOKit
  注册表通知即时感知 Magic Mouse 连接/断开，鼠标重连后秒级恢复，
  不再依赖健康检查轮询（稳定后间隔最长 300 秒）；轮询保留作为兜底

### 优化改进 🚀

- **降低点击/拖拽延迟**：新增 `EventSynthesizer`，点击与拖拽事件
  从多点触控队列直接发送，不再经过主线程跳转，避免菜单跟踪、
  模态弹窗等 UI 工作阻塞时造成的卡顿；点击与拖拽统一使用共享的
  hidSystemState 事件源，click state 全局一致

### 测试 🧪

- 新增 `EventSynthesizerTests`（11 个用例：点击/右键/双击 clickState、
  拖拽生命周期、重复开始/未开始即结束、取消语义）
- 新增 `MultitouchDeviceMonitorTests`（生命周期幂等性冒烟测试）
- 全套 67 个单元测试通过
