...
在菜单栏开启 **Two-Finger Zoom**，然后用两根手指在 Magic Mouse 表面同向上下滑动，即可向支持捏合手势的应用发送连续缩放。默认关闭；与 **Tap to Click** 独立，可单独启用。

默认向鼠标前端滑动放大，向后滑动缩小。在 **Sensitivity Settings…** 中可反转方向或调整缩放灵敏度（0.5×–2.0×）。两指都需要明显纵向移动；一指静止、横向移动或第三指加入不会启动缩放。结束后先抬起所有手指再开始下一次操作。

Chrome 每次缩放开始时会增加约 24–34 毫秒的短暂准备间隔，之后按触摸更新连续输出；期间的位移会保留。该调整正在实机验证。若遇到缩放跳变，可运行 `bash diagnose-zoom.sh` 记录本地时序，帮助对比鼠标和触摸板的行为。

首版目标是 Safari、Chrome 和“预览”的图片/PDF；应用实测与手指方向校准仍待完成，不能保证所有应用或 macOS 版本兼容。实现使用未公开缩放事件字段，不提供快捷键替代模式，也不是系统全屏放大。允许鼠标与触控板交替使用，但双指候选/缩放期间同时操作其他设备的滚动可能被短暂拦截。

如果菜单显示缩放暂停，普通滚动继续可用；确认辅助功能权限后将缩放关闭再开启。开发诊断与验收步骤见 [TESTING.md](TESTING.md#two-finger-zoom-validation)。
...
要先测试最新代码再决定是否安装，运行 `bash test-and-install.sh`。它每次重新构建并启动本次生成的 `build/MagicTapper.app`；确认安装前保留已安装版本。仅构建可用 `bash test-and-install.sh --build-only`，仅试运行可用 `--test-only`。`quick-test.sh` 和 `debug-run.sh` 也会重新构建，避免复用旧测试包。
