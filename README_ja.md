...
メニューバーの **Two-Finger Zoom** を有効にすると、Magic Mouse 上で2本の指を同じ方向に縦に動かし、対応アプリに連続ピンチズームを送信できます。初期設定はオフで、**Tap to Click** とは独立しています。

前方で拡大、後方で縮小する設計です。**Sensitivity Settings…** で方向の反転と感度（0.5×–2.0×）を調整できます。両方の指が縦に移動する必要があります。次の操作の前にすべての指を離してください。

Chrome では各ジェスチャーの開始時に約 24–34 ミリ秒の準備時間を設け、その間の移動量を保持します。その後は指の動きに合わせて連続出力します。この調整は実機検証中です。急な拡大・縮小が続く場合は `bash diagnose-zoom.sh` でローカルの時系列ログを記録できます。

Safari、Chrome、プレビューの画像/PDFを検証対象としていますが、実機テストと方向の校正は未完了です。非公開のジェスチャーイベントを利用し、画面全体の拡大やキーボードショートカットへの変換は行いません。マウスとトラックパッドの交互使用に対応しますが、同時操作では別デバイスのスクロールも一時的に抑止される場合があります。

ズーム停止の表示が出た場合は、アクセシビリティ権限を確認し、ズームをオフ・オンしてください。[検証手順](TESTING.md#two-finger-zoom-validation)を参照してください。
...
- 🖱️ **ダブルタップして離す** → システムのダブルクリック
- 🎯 **2回目を押したまま移動** → ドラッグ＆ドロップ
- ⚡ **高速レスポンス** - 遅延を最小限に最適化
...
スクリプトが行うこと：
- 毎回、現在のソースから最適化版を再ビルド
- 実行中のインスタンスを停止
- 旧バージョンをバックアップ
- /Applications にインストール
- 初回起動をガイド

インストール前に最新コードを試すには `bash test-and-install.sh` を実行します。毎回再ビルドし、生成した `build/MagicTapper.app` を起動します。インストールを承認するまで既存のアプリは保持されます。ビルドのみなら `--build-only`、試用のみなら `--test-only` を指定してください。`quick-test.sh` と `debug-run.sh` も起動前に再ビルドします。
